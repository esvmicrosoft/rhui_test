...
def validate_ca_certificates():
    ...
    """
    logging.debug('Entering validate_ca-certificates()')
    ...
    try:
        ...
        logging.error('Unable to check server side certificates installed on the server')
        logging.error('Use {} to reinstall the ca-certificates'.format(reinstall_ca_bundle_link))
        
        exit(1)
   
    if result:
        logging.error('The ca-certificate package is invalid, you can reinstall follow {} to reinstall it manually'.format(reinstall_ca_bundle_link))
        exit(1)
    else:
        return True
...
def connect_to_host(url, selection, mysection):
    ...
    try:
        ...
        logging.critical('Unable to identify OS version')
        exit(1)    
    ...
    logging.debug('baseurl for repo {} is {}'.format(mysection, url))
    ...
    try:
        ...
        logging.warning('Client certificate and/or client key attribute not found for {}, testing connectivity w/o certificates'.format(mysection))
        cert=()

    try:
        r = s.get(url, cert=cert, headers=headers, timeout=5, proxies=local_proxy)
    except requests.exceptions.Timeout:
        logging.warning('TIMEOUT: Unable to reach RHUI URI {}'.format(url))
        return False
        ...
        logging.warning('PROBLEM: Unable to use the proxy gateway when connecting to RHUI server {}'.format(url))
        return False
    except requests.exceptions.ConnectionError as e:
        logging.warning('PROBLEM: Unable establish connectivity to RHUI server {}'.format(url))
        ...
        logging.warning('PROBLEM: Unknown error, unable to connect to the RHUI server {}'.format(url))
        raise(e)
        return False
    else:
        if r.status_code == 200:
            ...
            logging.warning('The RC for this {} link is {}'.format(url, r.status_code))
            return False
...
def rpm_names():
    ...
    if rpm_names:
        for rpm in rpm_names:
            logging.debug('Server has this RHUI pkg: {}'.format(rpm))
        return(rpm_names)
    else:
...
def verify_pkg_info(package_name, rpm_info):
    ...
    if errors:
        ...
        logging.critical('follow {} for information to install the RHUI package'.format(data_link))
        exit(1)
...
def expiration_time(cert_path):
    ...
    if not default_policy():
        logging.critical('Client crypto policies not set to DEFAULT')
...
def read_yum_dnf_conf():
    ...
    logging.debug('Entering read_yum_dnf_conf()')
    ...
    try:
        ...
        logging.error('Problems reading /etc/yum.conf, on RHEL8+ it is a symbolic link to /etc/dnf/dnf.conf')
        raise
...
def get_proxies(parser_object, mysection):
    ...
    for key in ['proxy', 'proxy_user', 'proxy_password']:
        try:
            ...
            logging.error('Problems handling the parser object')
            raise
        else:
    ...
    try:
        ...
        if myproxy:
            ...
            if proxy_match:
                ...
                logging.critical('Invalid proxy configuration, please make sure it is a valid one')
                exit(1)
        else:
            return system_proxy
    except KeyError:
    ...
    if proxy_match.group(4) and ('proxy_user' in proxy_info.keys()):
        logging.warning('proxy definition already has a username defined and proxy_user is also defined, there might be conflicts using the proxy, repair')

    if proxy_match.group(6) and ('proxy_password' in proxy_info.keys()):
        logging.warning('proxy definition already has a username and password defined and proxy_password is also defined, there might be conflicts using the proxy, repair')

    if ('proxy_password' in proxy_info.keys() and proxy_info['proxy_password'])  and ('proxy_user' not in proxy_info.keys()):
        logging.warning('proxy_password defined but there is no proxy user, this could be causing problems')
        logging.warning('ignoring proxy_password')
    ...
    logging.warning('Found proxy information in the config files, make sure connectivity works thru the proxy')
...
def check_rhui_repo_file(path):
    ...
    try:
        ...
        logging.debug('{}'.format(str(reposconfig.sections())))
        return reposconfig
...
def check_repos(reposconfig):
    ...
    for repo_name in reposconfig.sections():
        ...
        if re.match(rhuirepo, repo_name):
            ...
            if enabled:
                ...
                logging.critical('Microsoft RHUI repository not enabled, please enable it with the following command')
    ...
    if not microsoft_reponame:
        ...
        logging.critical('Microsoft RHUI repository not found, reinstall the RHUI package following{}'.format(reinstall_link))
...
def ip_address_check(host):
    ...
    try:
        ...
        if rhui_ip_address  in rhui4:
            logging.debug('RHUI host {} points to RHUI4 infrastructure'.format(host))
            return True
            ...
            logging.error('RHUI server {} points to decommissioned infrastructure, reinstall the RHUI package'.format(host))
            logging.error('for more detailed information, use: {}'.format(reinstall_link))
            bad_hosts.append(host)
            warnings = warnings + 1
            return False
        else:
            logging.critical('RHUI server {} points to an invalid destination, validate /etc/hosts file for any invalid static RHUI IPs or reinstall the RHUI package'.format(host))
            logging.warning('Please make sure your server is able to resolve {} to one of the ip addresses'.format(host))
            ...
            logging.warning('listed in this document {}'.format(rhui_link))
            return False
    except Exception as e:
         logging.warning('Unable to resolve IP address for host {}'.format(host))
         logging.warning('Please make sure your server is able to resolve {} to one of the ip addresses'.format(host))
         rhui_link = 'https://learn.microsoft.com/azure/virtual-machines/workloads/redhat/redhat-rhui?tabs=rhel7#the-ips-for-the-rhui-content-delivery-servers'
         logging.warning('listed in this document {}'.format(rhui_link ))
         logging.warning(e)
...
def connect_to_repos(reposconfig, check_repos):
    ...
    logging.debug('Entering connect_to_repos()')
    rhuirepo = '^rhui-microsoft.*'
    warnings = 0
    ...
    for repo_name in check_repos:
        ...
        try:
            ...
            reinstall_link = 'https://learn.microsoft.com/troubleshoot/azure/virtual-machines/linux/troubleshoot-linux-rhui-certificate-issues?source=recommendations&tabs=rhel7-eus%2Crhel7-noneus%2Crhel7-rhel-sap-apps%2Crhel8-rhel-sap-apps%2Crhel9-rhel-sap-apps#solution-2-reinstall-the-eus-non-eus-or-sap-rhui-package'
            logging.critical('The baseurl is a critical component of the repository stanza and it is not found for repo {}'.format(repo_name))
            logging.critical('Follow this link to reinstall the Microsoft RHUI repo {}'.format(reinstall_link))
            exit(1)
        ...
        if successes == 0:
            ...
            logging.critical('PROBLEM: Unable to establish communication with one of the current RHUI servers, please ensure the server is able to resolve to a valid IP address and communication is allowed to the addresses listed in the public document {}'.format(error_link))
            sys.exit(1)
...
logging.critical('All communication tests to the RHUI infrastructure have passed, if problems persisit, remove third party repositories and test again')
logging.critical('The RHUI repository configuration file is {}, move any other configuration file to a temporary location and test again'.format(data['repofile']))
